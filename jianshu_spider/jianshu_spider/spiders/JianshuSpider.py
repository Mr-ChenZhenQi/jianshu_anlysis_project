...
class JianshuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for user_href in userlist:
            ...
            yield Request(url='http://www.jianshu.com/u/{slug}'.format(slug=user_slug),headers=self.base_headers,callback=self.parse_user,meta={'slug': user_slug})
            yield Request(url='http://www.jianshu.com/users/{slug}/followers'.format(slug=user_slug),headers = self.ajax_headers,callback = self.parse_followers,meta = {'slug': user_slug, 'page': 1})
